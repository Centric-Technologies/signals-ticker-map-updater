...
OVERRIDE_DB = pathlib.Path("static", "eodhd-overrides.csv")
...
def _get_historical_universe_bbg():
    logger.info(f"_get_historical_universe_bbg, read historical targets from network")
...
def _get_live_universe_bbg():
    logger.info(f"_get_live_universe_bbg, read from network")
...
def _get_yahoo_map():
    logger.info(f"_get_yahoo_map, read from network")
    ticker_map = (
...
def _build_eodhd_map(bbg_universe: set) -> pd.DataFrame:
    map_bbg_to_yahoo, _ = _get_yahoo_map()
    logger.info(f"map_bbg_to_yahoo, unique symbols:{len(map_bbg_to_yahoo)}")

    overrides_df = pd.read_csv(OVERRIDE_DB)
    overrides_dict = dict(zip(overrides_df["old"], overrides_df["new"]))
    logger.info(f"overrides_dict, count:{len(overrides_dict)}")
    ...
    for bbg_ticker in bbg_universe:
        ...
        if converter is not None:
            ...
            if converter.data_provider == MAP_IGNORE:
                pass
            elif converter.data_provider == MAP_YAHOO and bbg_ticker in map_bbg_to_yahoo:
        ...
        eodhd_map_list.append(
            {
                ...
                "ticker": signals_ticker,
            }
        )

    map = (
        pd.DataFrame(eodhd_map_list)
...
def complete_numerai_mapping():
    ...
    logger.info(f"historical_universe_bbg, unique symbols:{len(historical_bbg)}")

    live_bbg = _get_live_universe_bbg()
    logger.info(f"live_universe_bbg, unique symbols:{len(live_bbg)}")

    bbg_universe = _get_complete_universe_bbg(historical_bbg, live_bbg)
    logger.info(f"complete_universe_bbg, unique symbols:{len(bbg_universe)}")

    return _build_eodhd_map(bbg_universe)
